    ...
    >
      ...
      <div className="container mx-auto px-6 flex flex-col md:flex-row items-center justify-end relative z-10 h-full">
        <div className="w-full md:w-auto mb-6 md:mb-0 flex flex-col justify-center items-start md:pr-8 lg:pr-16 lg:pl-8 text-left">
          ...
          >
            {/* "Velkommen til" in white */}
            {text
              .slice(0, text.indexOf("Instacruit"))
              .split("")
              .map((char, index) => (
                <motion.span
                  key={index}
                  className="text-white" // Make "Velkommen til" white
                  variants={letter}
                >
                  {char}
                </motion.span>
            ...
            {/* "Instacruit" in purple */}
            {text
              .slice(text.indexOf("Instacruit"))
              .split("")
              .map((char, index) => (
                <motion.span
                  key={index}
                  className="text-[#830e70]" // Make "Instacruit" purple
                  variants={letter}
                >
                  {char}
                </motion.span>
          ...
          <div className="flex flex-row justify-start space-x-4 mt-1">
            <motion.a
              href="/request_page"
              className="bg-[#830e70] hover:bg-[#967e96] text-white font-bold py-4 px-6 md:px-10 rounded-xl transition duration-300"
              initial={{ opacity: 0, y: 50 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            <motion.button
              ...
              className="bg-white text-center hover:bg-[#bd99bd] text-[#830e70] font-bold py-3 px-4 md:px-8 rounded-xl transition duration-300"
              initial={{ opacity: 0, y: 50 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, ease: "easeOut", delay: 0.3 }}
              viewport={{ once: false, amount: 0.2 }}
